...
def main(page: ft.Page):
    ...
    def calculer_decoupe(e):
        ...
        result_area.controls.append(
            ft.Text(f"Nombre total de barres nécessaires: {nombre_de_barres}")
        )
        for i, barre in enumerate(barres):
            ...
            result_area.controls.append(
                ft.Text(f'  Longueur de la chute: {barre["chute"]}')
            )
        result_area.controls.append(
            ft.Text(f"Longueur totale de la chute: {chute_totale}")
        )
